...
if ($FilterType==3) // 1) + včetně creatora
...
if ($FilterType==2) {// podle volby redaktora
    ...
    elseif ($FilterID==-2)
    $sql="select A.ID, A.Edition, A.Title, A.Status, CC.color from RSP_ARTICLE  A left join `RSP_CC_ARTICLE_Stat` CC on A.Status=CC.ID where status not in (5,11)";
}
